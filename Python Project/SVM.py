    ...
    def __init__(self, filename: str) -> None:
        ...
        with open(filename) as file_obj:
            for line in file_obj.readlines():
                ...
                nums = line.split(',')
                age.append(float(nums[0]))
                estimateSalary.append(float(nums[1]))
...
class Perceptron:
    ...
    def Solve(self) -> np.ndarray:
        ...
        for i in trange(0, self.iteration):
            self.loss_per_iteration.append(self.Loss())
            if (self.Accuracy() >= 0.9999999): break
            gradient = self.Gradient()
            for index in range(0, len(self.weight)): self.weight[index] = self.weight[index] + self.learningRate * gradient[index]
            if (self.Accuracy() >= 0.9999999): break
        self.Draw()
...
logic_regression = LogicRegression("data2.csv")
logic_regression.Solve()

# perceptron = Perceptron("data2.csv")
# perceptron.Solve()